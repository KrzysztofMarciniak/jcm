...
enum JCMCategory { JCM_LOGIC, JCM_COMPARISON, JCM_ARITHMETIC, JCM_CALCULUS, JCM_CONTROL, JCM_FUNCTION, JCM_BINDING, JCM_MEMORY, JCM_IO };
enum JCMFormKind { JCM_PRIMITIVE, JCM_SPECIAL_FORM };
enum JCMValueKind { JCM_VALUE_NUMBER, JCM_VALUE_BOOLEAN, JCM_VALUE_FUNCTION, JCM_VALUE_STRING };
#define JCM_MACHINE_BITS 64
#define JCM_FALSE 0
#define JCM_TRUE 1

enum JCMOp {
    JCM_AND, JCM_OR, JCM_NOT, JCM_EQ, JCM_LT, JCM_LE, JCM_GT, JCM_GE,
    JCM_ADD, JCM_SUB, JCM_MUL, JCM_DIV, JCM_MOD,
    JCM_SUM, JCM_PRODUCT, JCM_INTEGRAL,
    JCM_IF, JCM_LAMBDA, JCM_BIND, JCM_LOAD, JCM_STORE, JCM_INPUT, JCM_OUTPUT
};

struct JCMCoreForm { enum JCMOp op; enum JCMCategory category; enum JCMFormKind kind; const char *name; const char *symbol; int min_args; int max_args; const char *syntax; int returns_boolean; int special_evaluation; };
...
static const struct JCMCoreForm jcm_core[] = {
    ...
    { JCM_SUM, JCM_CALCULUS, JCM_SPECIAL_FORM, "sum", "∑", 4, 4, "(∑ I START END BODY)", 0, 1 },
    { JCM_PRODUCT, JCM_CALCULUS, JCM_SPECIAL_FORM, "product", "∏", 4, 4, "(∏ I START END BODY)", 0, 1 },
    { JCM_INTEGRAL, JCM_CALCULUS, JCM_SPECIAL_FORM, "integral", "∫", 4, 4, "(∫ F START END STEPS)", 0, 1 },
...
#define JCM_BOOLEAN_RESULT(op) ((op) == JCM_AND || (op) == JCM_OR || (op) == JCM_NOT || (op) == JCM_EQ || (op) == JCM_LT || (op) == JCM_LE || (op) == JCM_GT || (op) == JCM_GE)
static const struct JCMCoreForm *jcm_core_find(const char *symbol) { int i; for (i=0;i<JCM_CORE_COUNT;i++) if (!strcmp(jcm_core[i].symbol,symbol)) return &jcm_core[i]; return NULL; }
static const struct JCMCoreForm *jcm_core_find_op(enum JCMOp op) { int i; for (i=0;i<JCM_CORE_COUNT;i++) if (jcm_core[i].op==op) return &jcm_core[i]; return NULL; }
#endif
